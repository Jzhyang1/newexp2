// Dumps the key (and, for SCAN ops, scan_length) sequence our workloads/workload.cpp
// generates, so it can be diffed against reference_dump.cpp built from the pinned
// My-YCSB commit. See run.sh for how this is invoked.
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <string>
...
int main(int argc, char** argv) {
    ...
    Workload* w = nullptr;
    // Only READ/SCAN are ever produced (see app.cpp's rationale comment): this keeps
    // the comparison well-posed, since generate_value_string()'s extra rand_r() draws
    // for UPDATE/INSERT/RMW would otherwise desync the two RNG streams independent of
    // the key-generation formulas we're actually trying to verify.
    OpProportion op_prop{0, 0, static_cast<float>(1.0 - a.scan_ratio), static_cast<float>(a.scan_ratio), 0};

    if (a.workload == "zipfian") {
        w = new ZipfianWorkload(a.value_size, a.scan_length, a.nr_entry, a.nr_op, op_prop, a.alpha, a.seed);
    } else if (a.workload == "uniform") {
        w = new UniformWorkload(a.value_size, a.scan_length, a.nr_entry, a.nr_op, op_prop, a.seed);
    } else if (a.workload == "latest") {
        w = new LatestWorkload(a.value_size, a.nr_entry, a.nr_op, a.read_ratio, a.alpha, a.seed);
    } else {
    ...
    while (w->has_next_op()) {
        ...
        w->next_op(&op);
        if (op.type == SCAN) {
            std::printf("SCAN %llu %ld\n", static_cast<unsigned long long>(op.key), op.scan_length);
        } else {
            std::printf("%s %llu\n", operation_type_name[op.type], static_cast<unsigned long long>(op.key));
        }
    }
    return 0;
}
